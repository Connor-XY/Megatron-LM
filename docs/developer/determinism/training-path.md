...
| Step | Where | Determinism | Notes |
| --- | --- | --- | --- |
| End-of-backward TP / PP gradient synchronization | `distributed/finalize_model_grads.py` | 🟡 | Sequence-parallel and Q/K-layernorm grads use native TP SUM; parameters explicitly marked for TP averaging use native AVG; tied word/position embeddings and replicated conditional/Flextron-router parameters use native PP all-reduce. These floating-point reductions remain topology-sensitive because deterministic mode does not replace them with a fixed rank order. The opt-in trace now fingerprints every existing collective input/output, including the token-count PP broadcast and DP sum, without adding communication or waits. Nemotron-3-Ultra's TP2×PP3 recipe exercises this boundary; the same-allocation 192-GPU control tests repeatability, not cross-topology equivalence. |
...
| Megatron-FSDP gradient reduction | `distributed/fsdp/src/megatron_fsdp/param_and_grad_buffer.py` | 🔴 | The synchronous helpers and overlapped bucket pipeline call native all-reduce/reduce-scatter directly, including optional native AVG and premultiplied SUM. They do not consume `reduce_scatter_with_fp32_accumulation`; deterministic mode therefore has no topology-independent Megatron-FSDP gradient path. Existing FSDP8 model cells repeat twice within one initialized topology and must not be interpreted as cross-allocation certificates. The production Ultra launcher does not enable Megatron-FSDP. |
...
| Structured runtime trace | `core/determinism_trace.py`, `extensions/transformer_engine.py`, `training.py`, `tensor_parallel/random.py`, `tensor_parallel/mappings.py`, `tensor_parallel/layers.py`, `pipeline_parallel/p2p_communication.py`, `distributed/param_and_grad_buffer.py`, `distributed/finalize_model_grads.py` | 🟢 | Opt-in rank-local events add no collectives or waits. Semantic comparison can hash paired forward/recompute outputs for Megatron checkpoint, Transformer Engine checkpoint, and output-discarding checkpoint paths; synchronous TP mapping collectives; core TP linear synchronous/async collectives at existing waits; MoE EP all-to-all; pipeline P2P; DP gradient-reduction; distributed-optimizer parameter-gather; final TP/PP gradient and token-count synchronization; and optimizer boundary tensors on selected iterations. TP userbuffer payloads remain open. |
