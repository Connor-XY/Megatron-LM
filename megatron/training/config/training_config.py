...
class TrainingConfig:
    ...
    determinism_trace_device_hashes: bool = False
    """Include compact 128-bit tensor-content digests computed on the tensor device. This avoids
    full device-to-host tensor copies, but is diagnostic evidence rather than a cryptographic
    full-byte certificate. Mutually exclusive with determinism_trace_tensor_hashes."""

    determinism_trace_summary_only: bool = False
    """Record only runtime identity, training-phase boundaries, reduced losses, and optimizer
    scalars. This is the low-cost all-rank tier for localizing a divergent rank/iteration;
    it is diagnostic evidence, not a full semantic trace certificate."""

    determinism_trace_rank_spec: str | None = None
    """Ranks that write traces: ``all`` or a comma-separated list/range such as ``0,3,8-15``.
    None selects all ranks. Sampling ranks is intended for large-scale localization, not an
    all-rank determinism certificate."""

    determinism_trace_flush_interval: int = 1
    """Flush each rank-local trace after this many events. One preserves crash-prefix durability;
    larger values reduce filesystem metadata and synchronization overhead."""

    determinism_trace_max_events_per_rank: int | None = None
    """Stop recording ordinary events after this per-rank count. A truncation marker and terminal
    iteration event are still written, and certification fails closed on any truncation."""

    determinism_trace_optimizer_state: bool = False
    """Include local optimizer parameter and state digests before and after each traced step."""
