...
# Determinism validation evidence (maintainer reference)

> **Audience:** maintainers who need to reproduce, audit, or extend determinism
> validation. This ledger intentionally retains internal cluster labels, job identifiers,
> artifact hashes, and workstream shorthand. It is not a user guide and is deliberately
> excluded from the user-facing documentation path.
>
> For the supported setup and limitations, see
> [Deterministic Training](../../user-guide/deterministic-training.md). For the concise
> developer status, see [status.md](./status.md).
