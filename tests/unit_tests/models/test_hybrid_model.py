...
def test_hybrid_schedule_allows_full_iteration_graph_for_ungrouped_pattern(monkeypatch):
    ...
    monkeypatch.setattr(
        TransformerModelChunkSchedulePlan,
        "__init__",
        lambda self, model, *args, **kwargs: initialized.append(model),
    )
    model = SimpleNamespace(
        config=SimpleNamespace(cuda_graph_impl="full_iteration"),
        decoder=SimpleNamespace(layer_type_list=["M", "*", "E"]),
    )
...
def test_hybrid_schedule_rejects_full_iteration_graph_for_grouped_pattern(monkeypatch):
    from megatron.core.models.common.model_chunk_schedule_plan import (
        TransformerModelChunkSchedulePlan,
    )
    from megatron.core.models.hybrid.model_chunk_schedule_plan import (
    ...
    monkeypatch.setattr(
        TransformerModelChunkSchedulePlan, "__init__", lambda self, model, *args, **kwargs: None
    )
    model = SimpleNamespace(
        config=SimpleNamespace(cuda_graph_impl="full_iteration"),
        decoder=SimpleNamespace(layer_type_list=[("M", "*", "E")]),
    ...
    with pytest.raises(AssertionError, match="does not support cuda graphs"):
        HybridStackModelChunkSchedulePlan(model)
