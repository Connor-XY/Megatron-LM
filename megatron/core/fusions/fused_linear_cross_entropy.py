...
class LinearCrossEntropy(torch.autograd.Function):
    ...
    def forward(
        ...
        with torch.cuda.nvtx.range("LinearCrossEntropy-forward"):
            is_distributed_weight = getattr(weight, "is_distributed_weight", False)
            materialized_weight = (
                weight.materialize_group_for_forward() if is_distributed_weight else weight
            )
            (
                logprobs,
                _maximum,
            ...
            ctx.is_distributed_weight = is_distributed_weight
    ...
    def backward(
        ...
        with torch.cuda.nvtx.range("LinearCrossEntropy-backward"):
            ...
            materialized_weight = (
                weight.materialize_group_for_backward(nvtx_label="linear_cross_entropy")
                if ctx.is_distributed_weight
                else weight
            )
            ...
            if ctx.is_distributed_weight:
                d_weight = weight.finalize_group_grads(
                    d_weight, nvtx_label="linear_cross_entropy"
                )
