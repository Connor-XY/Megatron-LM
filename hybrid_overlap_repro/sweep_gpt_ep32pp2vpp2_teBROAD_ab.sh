#!/bin/bash
#SBATCH -A nemotron_sw_pre -p batch -q short --nodes=16 --gres=gpu:4 --ntasks-per-node=4 -t 1:50:00 -J gptep32pp2v2teB
...
export RECOMPUTE_MODULES='core_attn moe_act shared_experts' OFFLOAD=0 EP_OVERRIDE=32 PP_OVERRIDE=2 NLPVPS=12 NUM_LAYERS=48 SEQ_LEN=3072
export RANKS_PER_NVL_DOMAIN=32 RUNTAG=gpt_ep32pp2vpp2_teBROAD
echo "== GPT EP32PP2VPP2 BASELINE @ $(date) =="; OVERLAP=0 bash lyris_proxy_train.sh
echo "== GPT EP32PP2VPP2 OVERLAP  @ $(date) =="; OVERLAP=1 bash lyris_proxy_train.sh
echo "== DONE @ $(date) =="
