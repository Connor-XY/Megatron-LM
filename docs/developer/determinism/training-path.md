...
| Routing map / probs construction | `moe_utils.py:823-837` | 🔵 | det: in-place `scatter_` for probabilities and the boolean map; top-k indices are unique within each row, so there are no write collisions. Non-det uses out-of-place `scatter`. The det path is byte-exact to the former `index_put_(accumulate=False)` implementation for outputs and gradients. Also `compute_routing_scores_for_aux_loss:890` and capacity masks `946/951` use collision-free `scatter` with **no** det branch. |
