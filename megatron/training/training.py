...
def _trace_summary_value(value):
    """Convert reduced loss structures into compact, stable JSON-ready values."""
    if isinstance(value, torch.Tensor):
        if value.numel() == 1:
    ...
    if isinstance(value, dict):
        return {
            str(key): _trace_summary_value(item)
            for key, item in sorted(value.items(), key=lambda item: str(item[0]))
        }
    if isinstance(value, (list, tuple)):
        return [_trace_summary_value(item) for item in value]
...
def _train_step(
    ...
    while rerun_state_machine.should_run_forward_backward(data_iterator):
        ...
        record_event(
            ...
            (
                {"losses_reduced": _trace_summary_value(losses_reduced)}
                if trace is not None and trace.summary_only
                else None
            ),
        )
        if save_activations_in_this_iteration:
    ...
    if mpu.is_pipeline_last_stage(ignore_virtual=True):
        ...
        for key in losses_reduced[0].keys():
            ...
            if val[0].numel() == 2:
                ...
                all_reduce_sum(val, group=mpu.get_data_parallel_group(with_context_parallel=True))
