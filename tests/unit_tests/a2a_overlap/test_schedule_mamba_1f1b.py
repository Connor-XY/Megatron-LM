...
results to the reference (non-overlapped) forward/backward for MambaModel
...
def build_mamba_model(config, hybrid_layer_pattern):
    """Build a MambaModel and input data for testing."""
    seq_len = 32
    max_seq_len = 300
...
class TestMambaA2AOverlapChunk:
    ...
    MambaModel.forward() for models with mixed Mamba + MoE layers.
    """
    ...
    def test_1f1b_schedule_mamba_model_chunk(self, dispatcher_type, layers_and_patterns):
        ...
        with deterministic_mode():
            for layer_num, pattern in zip(layers, patterns):
                ...
                model, data = build_mamba_model(config, pattern)
                model.cuda()
                models.append(model)
                datas.append(data)
