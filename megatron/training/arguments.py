...
def validate_args(args, defaults={}):
    ...
    if args.determinism_trace_dir is not None:
        ...
        if args.determinism_trace_optimizer_state:
            assert args.determinism_trace_tensor_hashes, (
                "--determinism-trace-optimizer-state requires " "--determinism-trace-tensor-hashes"
            )
        ...
        assert not args.determinism_trace_device_hashes
        assert not args.determinism_trace_summary_only
        assert args.determinism_trace_rank_spec is None
        assert args.determinism_trace_flush_interval == 1
        assert args.determinism_trace_max_events_per_rank is None
    ...
    if args.ddp_reduce_scatter_hierarchical_group_size is not None:
        ...
        assert args.use_distributed_optimizer, (
            "--ddp-reduce-scatter-hierarchical-group-size requires " "--use-distributed-optimizer."
        )
...
def _add_distributed_args(parser):
    ...
    group.add_argument(
        '--ddp-reduce-scatter-hierarchical-group-size',
        type=int,
        default=None,
