#!/bin/bash
#SBATCH -A nemotron_sw_pre -p batch -q short --nodes=16 --gres=gpu:4 --ntasks-per-node=4 -t 1:50:00 -J hybDBGtraj
...
export LOG_INTERVAL=1 NAN_OFF=1 EXIT_INTERVAL=8
...
export RANKS_PER_NVL_DOMAIN=32 RUNTAG=hybrid_dbg_traj
