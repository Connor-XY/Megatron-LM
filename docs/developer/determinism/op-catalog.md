...
| selected-only top-k sort experiment (GB300 / GB200) | AWS-CMH `721691`; AWS-DFW `541367`, 1 rank on 4-GPU allocations | console SHA256 `4bb415a0d48c…` / `93e4ec2c207b…`; script SHA256 `11ba03ce8141…` | `topk(sorted=False)` followed by `torch.sort(stable=False)` matches sorted values, indices, and input gradients exactly for random, quantized, and all-tied 256×top-k-8 and 512×top-k-22 cases. It is nevertheless slower in every shape: GB300 forward 1.07–3.84× and forward+backward 1.09–1.54×; GB200 forward 1.05–3.28× and forward+backward 1.06–1.59×. Rejected. |
| fused register-sort top-k experiment (GB200) | AWS-DFW `541420`, 1 rank on a 4-GPU allocation | console SHA256 `fd711bb5c46d…`; script SHA256 `bc6dfdca09c7…` | A Triton register sort after unsorted selection reproduces dense routing probabilities exactly, including quantized/all-tied scores, and is 2–3% faster forward at 4K+ tokens. Its extra autograd permutation makes forward+backward 1.01–1.85× slower, and quantized ties change 1,687/5,687 source-gradient elements at 256×top-k-8 / 512×top-k-22. Rejected. |
...
| final-gradient ordered-collective suite | Draco `10477843`, 8 H100 GPUs | console SHA256 `5bfd88b47bd7…`; verification SHA256 `18fc03851791…` | Every rank passes all 11 cases: fixed-order SUM/AVG in fp16/bf16/fp32 across multiple chunks and a padded tail, native and ordered trace metadata, and native two-rank exact SUM/AVG. |
| target-shape final-gradient cross-allocation certificate | AWS-DFW `539519` / `539562`, 16 GB200 GPUs each | certification SHA256 `2f68c50a9c14…`; cross-allocation comparison SHA256 `37b20a8878d0…` | TP2×PP2×DP4 training matches 1,856/1,856 semantic events with zero divergences, 64 exact recomputes, and zero pending collectives both within and across independent physical allocations. All 32 TP and 32 tied-embedding begin events per side select `native_two_rank_exact`, so the target TP2 path adds no communication. |
| ordered final-all-reduce primitive performance | AWS-DFW `539593`, 4 GB200 GPUs | `results.json`, SHA256 `207179058001…`; evidence manifest retained with the job | ABBA median-of-leg native/ordered latency is 0.245/0.374 ms for 64 MiB (1.53×, +0.130 ms) and 0.719/1.189 ms for 256 MiB (1.65×, +0.471 ms). Ordered 256 MiB peak allocation is 486.5 MB versus 268.4 MB native; additional workspace is a constant multiple of the configured 64 MiB chunk rather than the full tensor size. |
| TP4 ordered final-gradient cross-allocation certificate | AWS-DFW `539611` / `539639`, 16 GB200 GPUs each | cross-allocation comparison SHA256 `9da4be56d4cf…`; certification SHA256 `e4e906a63df2…` / `e7fd6dcd0f14…` | Both two-launch certificates and the independent-allocation comparison match 2,432/2,432 events with zero divergences, 128 exact recomputes, and zero pending collectives per run. All 32 traced final TP-gradient begin events per launch report `logical_rank_all_to_all_sum_all_gather`, group size 4, and fp32 accumulation. |
| post-final-gradient DSV3 + Nemotron EP32 certificates | AWS-DFW `539669` / `539674`, disjoint 32-GPU allocations | DSV3/Nemotron cross-allocation SHA256 `a5dfaaa3693b…` / `1df5a3d53be7…`; helper/finalizer SHA256 `fd8f9e665f00…` / `672b5314ec77…` | DeepSeek-V3 matches 6,720/6,720 events and Nemotron matches 10,304/10,304 events both within and across allocations. Both have exact recomputes, zero divergence or pending collectives, expert-bias trace coverage, and no DP reductions missing ordered or hierarchical fp32 accumulation. |
| fixed-order deterministic unpermute suite | Draco `10478095`, 8 H100 GPUs | console SHA256 `83ed77f13589…`; source helper/utils/dispatcher SHA256 `e26c1d92f08f…` / `5955a081d0f0…` / `6a715ff5b33e…` | Every rank passes all 42 fp16/bf16/fp32 cases across top-k 4/8/22 and aligned/non-aligned hidden sizes, including exact forward/backward equality to deterministic `index_add_`, guarded small-shape fallback, and CUDA-graph replay. |
| final-source A2A model matrix after unpermute | Draco `10478144`, 8 H100 GPUs | pytest SHA256 `5198620b1cb5…`; source-manifest SHA256 `a557388b5350…` | All eight ranks pass all 12 DeepSeek top-k-8 and Nemotron top-k-6 A2A cells, including TP2×EP4, PP/VPP, FSDP8×EP4, and FSDP8. The manifest contains the final guarded helper, utilities, and dispatcher hashes. |
| fixed-order deterministic unpermute ABBA | AWS-DFW `539787`, 1 rank on a 4-GPU GB200 allocation | `results.json`, SHA256 `decedfeca5dc…`; helper SHA256 `e26c1d92f08f…` | All four cells are byte-exact. At 512 tokens, candidate/native latency is 0.141/0.235 ms for DSV3 top-k 8 hidden 7,168 and 0.136/0.221 ms for Nemotron top-k 22 hidden 2,048 (1.66×/1.63× faster). At 4,096 tokens it is 0.215/0.966 and 0.192/0.804 ms (4.48×/4.18× faster); peak allocation falls 42–47%. |
| post-unpermute MoE profile | Draco `10478093`, 8 H100 GPUs | console SHA256 `1c49882b5af5…`; deterministic/nondeterministic reports SHA256 `174f60a256a0…` / `c774fff5d6db…` | The selected deterministic path contains `_DeterministicUnpermute` (32.414 ms across captured NVTX ranges) and no `aten::index_add_`. Iteration 7 is 226.9/191.8 ms deterministic/native (1.18×), passing the 1.25× gate. This is a healthy final-source profile, not a same-allocation before/after attribution. |
| unpermute cross-version DSV3 + Nemotron EP32 certificates | AWS-DFW baseline `539669` / final candidate `539876`, independent 32-GPU allocations | DSV3/Nemotron cross-version SHA256 `3587b7c9b2ab…` / `3cf0de1f8045…`; candidate certification SHA256 `882d0a98af9b…` / `e9c826ef816b…` | The optimized code matches all 6,720 DeepSeek-V3 and 10,304 Nemotron events against the pre-optimization code, with zero divergences, exact recomputes, zero pending collectives, and complete ordered hierarchical DP coverage. |
...
| TP2 native reduce-scatter qualification | AWS-CMH `723241` / `723271`; model jobs `723347` / `723417` | perf console SHA256 `e0afd6573729…`; equivalence console SHA256 `e53dd25e55f…`; cross-allocation DSV3/Nemotron reports SHA256 `e9ad3e6427e6…` / `8140c5b37444…` | Native two-rank reduce-scatter is 1.56–1.89× faster than the ordered helper across bf16/fp32 8/64/160 MiB and matches it in all 288 randomized fp16/bf16/fp32 sync/async cells. Independent TP2×EP16 allocations match 12,864/12,864 DSV3 and 17,984/17,984 Nemotron events; group-2 DP reductions report `native_two_rank_exact`, while group-16 reductions retain the hierarchy. |
| single-rank reduce-scatter identity | AWS-CMH benchmark `723921`; final-source model job `724148`; focused job `724146`; ABBA `723987` | results SHA256 `a2d2b7dc9843…`; DSV3/Nemotron certificates SHA256 `2e45875de123…` / `a6771a4d3f44…`; focused log SHA256 `4f6e03ef4565…`; ABBA analysis SHA256 `22e99a60c7a1…` | On traced TP1×EP32 target shapes, the old helper launched one 16.8M-element and two 41.9M-element identity reductions per rank/iteration. The no-op alias path reduces synchronized wall time from 0.143–0.252 ms to about 0.006 ms (23.5–42.4×). Same-allocation DSV3 ABBA improves median-of-leg medians 53.625→52.85 ms (-1.45%), with both order pairs positive and all 12 losses identical. DSV3/Nemotron match 6,848/6,848 and 10,432/10,432 events with every expert reduction labeled `single_rank_identity`; all four ranks pass 36 CPU/CUDA, aliased/distinct, sync/async cases. |
| actual TE attention-backend trace | AWS-CMH exact-source `723738`, 32 GPUs; AWS-DFW `544156`, one GB200 | DSV3/Nemotron certificate SHA256 `c55f5083bdbf…` / `c9e4d8359238…`; DFW trace SHA256 `61449b39961d…` | Both target models pass two-launch exact certification with 256 selected-backend events per run and zero unavailable events. DSV3 MLA uses unfused attention under auto; Nemotron uses fused `NVTE_F16_arbitrary_seqlen`; forward and recompute agree. The independent TE 2.17 GB200 probe also selects and records the same fused sub-backend under auto. |
...
- ✅ **Applied:** replace dropless, unpadded deterministic A2A unpermute
  `index_add_` with a fixed-order Triton segment sum and collision-free gather
  backward. The final-source isolated ABBA is 1.63–4.48× faster with 42–47%
  lower peak allocation; DSV3 and Nemotron cross-version traces remain exact.
  Other routing modes retain `index_add_`, and no full-step percentage is
  claimed until a balanced production ABBA separates overlap and control drift.
...
floating reductions inside TP mappings/linears/vocab-parallel cross-entropy and
the non-distributed-optimizer DP all-reduce.
