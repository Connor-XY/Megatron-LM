...
class HyperConnectionHybridLayer(GraphableMegatronModule):
    ...
    * MoE inner layers, when ``moe_router`` is in ``cuda_graph_modules``: the expert
    ...
    def _moe_raw_delta(self, aggregated: Tensor):
        """Eager raw MoE delta: run the inner layer's attention prefix (identity for MoE-only
        "E" layers) + pre-MLP layernorm + experts, returning ``mlp_output_with_bias`` WITHOUT
        the inner's residual-add. The mHC n-stream BDA owns the residual. Mirrors the graph
        path's ``resume_moe_experts_after_partial_cudagraph`` so eager == replay bit-identically.
        """
        inner = self.inner_layer
        hidden_a, _ = inner._forward_attention(aggregated, attention_mask=None)
        pre = inner._forward_pre_mlp_layernorm(hidden_a)
        if isinstance(pre, tuple):
            pre = pre[0]
        return inner.mlp(pre)
    ...
    def _te_cuda_graph_replay(self, *args, **kwargs):
        ...
        if self._inner_is_partial_moe_capture():
            ...
            # (`_moe_raw_delta` → fused_h_res_h_post_bda), just with the prefix graphed.
    ...
    def forward(
        ...
        if self._inner_is_moe():
            # GPT-style (mirrors HyperConnectionTransformerLayer): feed the inner MoE's raw
            # `mlp_output_with_bias` straight into the n-stream BDA, which owns the residual
            # combine. This avoids the lossy `layer_output - aggregated` bf16 subtraction and
            # keeps eager bit-identical to the CUDA-graph replay path
            # (resume_moe_experts_after_partial_cudagraph), which produces the same delta.
            layer_output_with_bias = self._moe_raw_delta(aggregated)
            context = None
        else:
            ...
            # The inner hybrid layer already applied its own local residual/dropout, so it
            # returns `aggregated + f(aggregated)`. Feed only the function delta `f(aggregated)`
            # into the n-stream BDA so it does not double-count the residual that mHC owns.
            ...
            # `fp32_residual_connection=True` may cause some inner layers (e.g., MambaLayer)
            # to return `layer_output` in fp32 while `aggregated` is in compute dtype; upcast
            # `aggregated` so the subtraction stays in fp32.
