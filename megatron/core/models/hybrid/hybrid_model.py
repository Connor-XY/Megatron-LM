...
class HybridModel(LanguageModule):
    ...
    def _preprocess(
        ...
        """Preprocesses inputs for the hybrid decoder.

        Applies embeddings to input tokens, or uses `decoder_input` from a previous
        pipeline stage. Also sets up rotary positional embeddings if configured.

        Returns:
            Tuple of (decoder_input, rotary_pos_emb, None, None, None, padding_mask)
            to match GPTModel._preprocess signature (rotary_pos_cos, rotary_pos_sin,
            sequence_len_offset are not used by HybridModel).
        """
        # Decoder embedding.
        ...
        if self.position_embedding_type == 'rope':
            rotary_seq_len = self.rotary_pos_emb.get_rotary_seq_len(
                None, self.decoder, decoder_input, self.config, packed_seq_params
            )
    ...
    def _postprocess(
        ...
        Applies MTP if enabled, generates output logits through the output layer,
        and computes language model loss when labels are provided.
        """
        output_weight = None
        if self.share_embeddings_and_output_weights:
            output_weight = self.shared_embedding_or_output_weight()

        mtp_forward_ran = self.mtp_process and mtp_in_postprocess
        if mtp_forward_ran:
    ...
    def build_schedule_plan(
        ...
        """Builds a computation schedule plan for the hybrid model.

        This enables A2A overlap for MoE layers in hybrid Mamba-Transformer patterns.
        The schedule plan decomposes each layer into fine-grained nodes that can be
        executed on separate CUDA streams, overlapping MoE A2A communication with
        Mamba/Attention computation.
