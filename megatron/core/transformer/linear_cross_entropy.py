...
class LinearCrossEntropyModule(tensor_parallel.ColumnParallelLinear):
    ...
    def _compute_linear_and_cross_entropy_loss(
        ...
        # Scaling the hidden states is algebraically equivalent to scaling the logits, while
        # retaining the fusion boundary and avoiding a materialized logits tensor.
        if self.config.use_mup and self.config.mup_output_mult != 1.0:
            hidden = hidden * self.config.mup_output_mult
