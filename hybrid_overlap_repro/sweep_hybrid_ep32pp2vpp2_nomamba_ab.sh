#!/bin/bash
#SBATCH -A nemotron_sw_pre -p batch -q short --nodes=16 --gres=gpu:4 --ntasks-per-node=4 -t 1:50:00 -J hybpp2v2nomamba
...
export MODEL=hybrid FSDP=0 TP_OVERRIDE=1 CUDA_GRAPH=te TE_CG_SCOPE='attn moe_router moe_preprocess' GBS=512 EXIT_INTERVAL=80
...
export RANKS_PER_NVL_DOMAIN=32 RUNTAG=hybrid_ep32pp2vpp2_nomamba
